...
const questions = [
  ['What do I need to run Quickque?', 'An Apple Silicon Mac running macOS 14 or newer. For now, you’ll need to build Quickque from source; there isn’t a ready-to-install download yet. The install guide walks through the requirements.'],
...
export function HomePage({ context }: { context?: any }) {
  ...
  return <div ref={rootRef} className="premium-home" data-motion-paused={motionPaused}>
    ...
    <section className="closing-section"><div className="closing-grid" aria-hidden="true" /><div className="site-container closing-content"><span className="eyebrow dark-eyebrow"><Sparkles size={14} /> START WITH A SCRIPT YOU KNOW</span><h2>Open your script.<br /><span>Give it a read.</span></h2><div className="hero-actions"><Link href="/pricing" className="premium-button button-dark">Compare plans <ArrowUpRight size={18} /></Link><Link href="/install" className="text-link">Build from source <ArrowRight size={17} /></Link></div><p className="closing-price">Free to use · Pro planned at {config?.commerce?.monthlyDisplayPrice}/month or {config?.commerce?.displayPrice} lifetime</p>{config?.release?.status === 'unavailable' && <p className="release-note">Available as a source build today. A Mac download is still in the works.<br />Voice features are still being tested on Mac.</p>}</div></section>
