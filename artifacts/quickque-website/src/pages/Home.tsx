...
export function HomePage({ context }: { context?: any }) {
  ...
  return <div ref={rootRef} className="premium-home" data-motion-paused={motionPaused}>
      ...
      <div className="hero-content site-container">
        ...
        <p className="hero-description">Keep your script in view for a talk, a video call or a rehearsal.<br className="desktop-break" /> Quickque handles the prompting. Your words and voice stay on your Mac.</p>
         <div className="hero-actions">
           {config?.release?.status === 'available' ? (
             <Link href="/install" className="premium-button button-light">Download Quickque <ArrowDown size={18} /></Link>
           ) : (
             <a href={config?.demoUrl} className="premium-button button-light">Open the live demo <ArrowUpRight size={18} /></a>
           )}
           <Link href="/pricing" className="premium-button button-ghost">See plans <ArrowUpRight size={16} /></Link>
         </div>
        <div className="hero-footnote"><span className="hero-platform"><Command size={13} /> Apple Silicon · macOS 26+</span><span className="hero-footnote-divider" /><span>Free to start. No account needed. {config?.release?.status === 'available' ? 'Test DMG available.' : 'Browser demo available. Mac build coming soon.'}</span></div>
      </div>
    ...
    <section className="use-cases-section site-container section-space"><div className="section-heading"><div><span className="eyebrow dark-eyebrow">YOUR WORKFLOW</span><h2>Give the talk.<br /><span>Practise the part.</span></h2></div><p>A speech and a scene need different kinds of prompting. Quickque gives you a place for both.</p></div><div className="use-case-grid"><article className="use-case-card presenting"><span>01 / PRESENTING</span><div className="use-case-art"><img src={`${config?.basePath ?? '/'}images/presentation-artwork.webp`} width="1200" height="900" alt="An empty lectern ready for a presentation" loading="lazy" /></div><h3>Your notes, beside the camera.</h3><p>Keep your place through a long presentation. Scroll by hand, set a steady speed or let Voice Follow move the script as you speak.</p><small><LockKeyhole size={13} /> Voice following runs on your Mac</small></article><article className="use-case-card rehearsing"><span>02 / REHEARSING</span><div className="use-case-art"><img src={`${config?.basePath ?? '/'}images/performance-artwork.webp`} width="1200" height="900" alt="A theatre stage prepared for a solo performance" loading="lazy" /></div><h3>Someone to read the other lines.</h3><p>Choose your character and give the other parts a voice. Scene Partner reads their lines while you practise yours, with speech generated on your Mac.</p><small><LockKeyhole size={13} /> No cloud AI required</small></article></div></section>
    <section id="in-action" className="product-section site-container section-space">
      <div className="section-heading"><div><span className="eyebrow dark-eyebrow">01 / PRESENTING</span><h2>Keep your notes.<br /><span>Lose the paper shuffle.</span></h2></div><p>Import a script, split it into sections and open the reader. Adjust the text size and pace until it feels comfortable.</p></div>
    ...
    <section className="library-section site-container section-space"><div className="library-copy"><span className="eyebrow dark-eyebrow">05 / PRIVATE BY DESIGN</span><h2>A home for<br /><span>your scripts.</span></h2><p>Import the documents you already have. Pin active work, see which scripts have generated audio and keep presentations and performances easy to recognise.</p><div className="file-types"><span>TXT</span><span>DOCX</span><span>RTF</span><span>PDF</span></div><Link href="/guide/scripts" className="text-link">See how scripts are saved <ArrowUpRight size={17} /></Link></div><div className="library-image" data-parallax="-0.035"><div className="image-label"><span className="status-dot" /> THE QUICKQUE WORKSPACE · ACTUAL APP VIEW</div><img src={`${config?.basePath ?? '/'}images/workspace-cards.webp`} width="1440" height="900" alt="Quickque workspace showing pinned presentation and performance script cards" loading="lazy" /></div></section>
    ...
    <section className="closing-section"><div className="closing-grid" aria-hidden="true" /><div className="site-container closing-content"><span className="eyebrow dark-eyebrow"><Sparkles size={14} /> START WITH A SCRIPT YOU KNOW</span><h2>Open your script.<br /><span>Give it a read.</span></h2><div className="hero-actions">
      {config?.release?.status === 'available' ? (
        <Link href="/install" className="premium-button button-dark">Download Quickque <ArrowDown size={18} /></Link>
      ) : (
        <a href={config?.demoUrl} className="premium-button button-dark">Open the live demo <ArrowUpRight size={18} /></a>
      )}
      <Link href="/pricing" className="text-link">Compare plans <ArrowRight size={17} /></Link>
    </div><p className="closing-price">Free to use · Pro planned at {config?.commerce?.monthlyDisplayPrice}/month or {config?.commerce?.displayPrice} lifetime</p><p className="release-note">{config?.release?.status === 'available' ? 'The current Mac download is an unsigned test build. Voice features are still being tested on Mac.' : 'The native Mac release is being prepared. Voice features are currently available to test in the browser demo.'}</p></div></section>
