  ...
  const result = await page.evaluate(async () => {
    ...
    (window as any).__TAURI_INTERNALS__ = { invoke: async (cmd: string) => {
      ...
      if (cmd === 'script_audio_entitlement') return { paid: true };
       if (cmd === 'script_audio_status') return { status: missing ? 'missing' : 'ready', revision: 'r', entries: [] };
    ...
    class Context {
      ...
      async close() { this.state = 'closed'; }
       async decodeAudioData() { return { length: 32, numberOfChannels: 1, duration: 0.08 }; }
      createBufferSource() {
        sources++;
    ...
    (window as any).AudioContext = Context;
     const request = { scriptId: 's', revision: 'r', entries: [
       { id: 'a', text: 'Hello', voiceId: 'chatterbox-turbo:default-en', rate: 1 },
       { id: 'b', text: 'Hello', voiceId: 'chatterbox-turbo:default-en', rate: 1 },
     ] };
    ...
    delete (window as any).__TAURI_INTERNALS__;
     return { reads: calls.filter(cmd => cmd === 'script_audio_read').length, duration, synthesis: calls.some(cmd => /speak|generate/.test(cmd)), sources, cancelled, lateCancelled, blocked };
  });
   expect(result).toEqual({ reads: 1, duration: 0.16, synthesis: false, sources: 2, cancelled: true, lateCancelled: true, blocked: true });
});
...
test('prepared audio progress follows the playback clock and stops at completion', async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    (window as any).__TAURI_INTERNALS__ = { invoke: async (cmd: string) => {
      if (cmd === 'script_audio_entitlement') return { paid: true };
       if (cmd === 'script_audio_status') return { status: 'ready', revision: 'r', entries: [] };
      if (cmd === 'script_audio_read') return [0];
      throw new Error(`Unexpected native call ${cmd}`);
    } };
    class Context {
    ...
    (window as any).__TAURI_INTERNALS__ = {
       invoke: async (command: string, args?: any) => {
        ...
        if (command === 'voice_library_list') return [];
         if (command === 'script_audio_status') return { status: 'ready', revision: args?.request?.revision, entries: [] };
...
test('audio panel shows truthful progress and retains it when reopened without status polling', async ({ page }) => {
  ...
  await page.evaluate(async () => {
    ...
    (window as any).__TAURI_INTERNALS__ = {
      ...
      invoke: async (command: string, args: any) => {
        ...
        if (command === 'script_audio_entitlement') return { paid: true };
        if (command === 'voice_library_list') return [];
         if (command === 'script_audio_status') { checks++; return { status: generated ? 'ready' : 'missing', revision: args.request.revision, entries: [] }; }
        if (command === 'script_audio_generate') {
          ...
          callbacks.get(handlers.get('script-audio-diagnostic')!)?.({ payload: { ...args.request, stage: 'model_load' } });
           return new Promise(resolve => { (window as any).finishAudio = () => { generated = true; resolve({ status: 'ready', revision: args.request.revision, entries: [] }); }; });
        }
        throw new Error(command);
      },
    };
  ...
  await page.getByRole('button', { name: 'Prepare audio', exact: true }).click();
   await expect(page.getByText(/Queued…|Checking audio setup…|Loading Chatterbox…/)).toBeVisible();
