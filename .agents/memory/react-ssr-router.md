...
For an app mounted at the domain root, pass Wouter an empty base prefix rather
than `/`; a slash base can hydrate server-rendered `/route` links as `//route`.
...
browser screenshot. For root mounts, also inspect hydrated link destinations.